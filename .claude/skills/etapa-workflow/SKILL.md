...
3. Consulte a doc atual da biblioteca pelo Context7 antes de usar APIs de faster-whisper, MediaPipe, LangChain, FastAPI, React/Vite ou rembg. Veja também as skills `ffmpeg-video`, `langchain-structured` e `mediapipe-face`.
...
6. Rode `uv run pytest -q` e `uv run python -m src.doctor` (dentro de `API/`). Se a etapa muda o que o usuário vê ou faz, exponha na API (`API/src/api/`, com testes em `tests/test_api.py`) e no frontend (`frontend/`, via subagente `frontend-engineer`).
